...
- **System Cache Manager (SQLite)**: On app startup, Rust backend scans all drives in background and writes to `system_index.db` (SQLite with WAL mode, indexed `name` column). Search queries the DB directly on disk — zero RAM usage. Progress emitted via Tauri events.
...
## Directory Map
- `src/tools/folder-searcher/` — System File & Folder Searcher with SQLite-based indexing.
...
- `src-tauri/src/lib.rs` — All Rust commands including `start_background_index`, `search_index`, `get_index_status`, `search_system`.
