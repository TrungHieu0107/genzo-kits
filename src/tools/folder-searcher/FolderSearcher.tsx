                  ...
                  <table className="w-full border-separate border-spacing-0" style={{ tableLayout: 'fixed' }}>
                    ...
                    <tbody>
                      ...
                      {virtualItems.map((virtualRow: VirtualItem) => {
                        ...
                        if (!item) return null;
                        return (
                          <tr key={virtualRow.key} className={`group hover:bg-[#2a2a2b] transition-colors text-sm ${selectedPaths.has(item.path) ? 'bg-emerald-500/5' : ''}`} style={{ height: `${virtualRow.size}px` }}>
