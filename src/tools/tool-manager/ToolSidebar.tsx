  ...
  return (
    <div className={`${isCollapsed ? "w-[68px]" : "w-[280px]"} h-screen bg-[#181818] border-r border-[#2d2d2d] flex flex-col flex-shrink-0 transition-all duration-300 relative`}>
      ...
      <div className={`p-4 border-b border-[#2d2d2d] flex items-center ${isCollapsed ? "justify-center" : "justify-between"}`}>
        <div className={`flex items-center gap-3 ${isCollapsed ? "hidden" : "flex"}`}>
          ...
          <div>
            ...
            <p style={fs.nano} className="text-gray-500">v1.0+</p>
          </div>
        </div>
        <button 
      ...
      <div className={`flex-1 overflow-y-auto ${isCollapsed ? "p-2" : "p-3"} flex flex-col gap-1 hide-scrollbar`}>
        {!isCollapsed && <div style={fs.caption} className="font-semibold text-gray-500 uppercase tracking-wider mb-2 px-2 whitespace-nowrap">Available Tools</div>}
              ...
              <div className={`flex items-center gap-3 ${isCollapsed ? "" : "w-full"}`}>
                ...
                {!isCollapsed && (
                  <div className="flex-1 min-w-0">
                    <div style={fs.bodySm} className="font-semibold truncate">{tool.name}</div>
                    <div style={fs.caption} className="text-gray-500 truncate mt-0.5">{tool.description}</div>
                  </div>
                )}
              </div>
              {/* Active Indicator */}
        ...
        {(() => {
          ...
          return (
            ...
            >
              ...
              {!isCollapsed && <span style={fs.bodySm} className="font-semibold">Settings</span>}
            </button>
          );
        })()}
      </div>
      ...
      {contextMenu && (
        <>
          ...
          >
            <button 
              className="w-full text-left px-4 py-2 text-gray-300 hover:bg-blue-600 hover:text-white transition-colors flex items-center gap-3"
              style={fs.caption}
              onClick={() => {
